...
# API endpoints
@app.get("/collatz/{num}", response_model=CollatzRecord,
...
def read_collatz(num: int, db: Session = Depends(get_db)):
    ...
    if data is None:
        ...
        return CollatzRecord(starting_number=data[0], number_of_steps=data[1], max_value=data[2],
                             sequence_length=data[3], convergence=data[4], timestamp=data[5])
...
def read_stats(db: Session = Depends(get_db)):
    c = db.execute('SELECT MAX(starting_number), MIN(timestamp), MAX(timestamp) FROM collatz')
    ...
    if last_number is None:
        return {"last_checked_number": 0, "total_computation_time": 0, "average_steps": 0}
    else:
        min_timestamp = datetime.datetime.strptime(min_timestamp_str, "%Y-%m-%d %H:%M:%S")
        max_timestamp = datetime.datetime.strptime(max_timestamp_str, "%Y-%m-%d %H:%M:%S")
        total_time = (max_timestamp - min_timestamp).total_seconds()
        return {"last_checked_number": last_number, "total_computation_time": total_time, "average_steps": average_steps}


@app.get("/collatz/range/{start}/{end}",
...
def read_collatz_range(start: int, end: int, db: Session = Depends(get_db)):
    c = db.execute('SELECT * FROM collatz WHERE starting_number BETWEEN ? AND ?', (start, end))
    data = c.fetchall()
    return [{"starting_number": x[0], "number_of_steps": x[1], "max_value": x[2],
             "sequence_length": x[3], "convergence": x[4], "timestamp": x[5]} for x in data]


@app.get("/collatz/top/{n}",
...
def read_average_collatz(n: int, db: Session = Depends(get_db)):
    c = db.execute('SELECT AVG(number_of_steps), AVG(max_value) FROM (SELECT * FROM collatz ORDER BY starting_number DESC LIMIT ?)', (n,))
    avg_steps, avg_max = c.fetchone()
    return {"average_number_of_steps": avg_steps, "average_max_value": avg_max}


@app.get("/collatz/search/{number_of_steps}/{max_value}",
         summary="Search Collatz Sequences",
         description="Search for Collatz sequences by a specific number of steps and max value.")
def read_search_collatz(number_of_steps: int, max_value: int, db: Session = Depends(get_db)):
    c = db.execute('SELECT * FROM collatz WHERE number_of_steps = ? AND max_value = ?', (number_of_steps, max_value))
    data = c.fetchall()
    return [{"starting_number": x[0], "number_of_steps": x[1], "max_value": x[2],
             "sequence_length": x[3], "convergence": x[4], "timestamp": x[5]} for x in data]


@app.get("/collatz/stats/hourly",
...
def read_hourly_stats(db: Session = Depends(get_db)):
    ...
    c = db.execute('SELECT COUNT(*) FROM collatz WHERE timestamp BETWEEN ? AND ?', (one_hour_ago, current_time))
    count = c.fetchone()[0]

    return {"collatz_count_last_hour": count}


@app.get("/collatz/stats/distribution",
...
@app.post("/refresh_block_list/{password}",
